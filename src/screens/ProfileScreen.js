...
//
// Changes vs original:
//   • Creator state now lives in CreatorContext (AsyncStorage-backed, global)
//   • CreatorFormScreen is fully controlled — all fields editable, validated
//   • POST /creators/apply wired with real API call + error handling
//   • Profile data refresh after creator activation
...
function HamburgerMenu({
  ...
  const menuItems = [
    { icon: "person-outline", title: "My Learning Profile", subtitle: "Monitor your progress", onPress: () => go("LearningProfileScreen") },
    { icon: "lock-closed-outline", title: "Subscription", subtitle: "Manage your plan", onPress: () => go("Subscription") },
  ];

  const creatorDashboardItems = [
    { icon: "grid-outline", title: "Dashboard", subtitle: "Full analytics & channel overview", onPress: () => go("CreatorDashboard") },
    ...
    { icon: "book-outline", title: "Manage Courses", subtitle: "Edit and publish your courses", onPress: () => go("ManageCourses") },
  ];
    ...
    <Modal transparent visible={visible} animationType="none" onRequestClose={onClose}>
      ...
      <Animated.View style={[drawerStyles.drawer, { transform: [{ translateX: slideAnim }] }]}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
          ...
          {/* Creator Dashboard — creators only */}
          {isCreator && (
            <>
...
// ─────────────────────────────────────────────────────────────────────────────
// Creator Application Form — fully controlled + wired to API
// ─────────────────────────────────────────────────────────────────────────────
function CreatorFormScreen({ onBack, onSubmit, prefillName }) {
  ...
  const validate = () => {
    ...
    if (!whyTeach.trim() || whyTeach.trim().length < 20) e.whyTeach = "Please write at least 20 characters";
    return e;
  };
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      // Try known routes; if the server returns 404 we fall through to the fallback
      let res;
      const payload = {
        ...
        subjects,
        experience,
        motivation: whyTeach.trim(),
      };

      try {
        res = await api.post("/creators/apply", payload);
      } catch (firstErr) {
        // If 404, try alternate common route
        if (firstErr?.response?.status === 404) {
          res = await api.post("/users/creator-apply", payload);
        } else {
          throw firstErr;
        }
      }

      // Pass back server response so ProfileScreen can store it in context
      onSubmit({
        displayName: displayName.trim(),
        tagline: tagline.trim(),
        subjects,
        experience,
        creatorId: res?.data?.creatorId ?? res?.data?._id ?? null,
        channelName: res?.data?.channelName ?? displayName.trim(),
      });
    } catch (err) {
      const msg =
    ...
    <SafeAreaView style={formStyles.container}>
      ...
      >
        ...
        <View style={formStyles.field}>
          ...
          <TextInput
            style={[formStyles.inputEditable, errors.displayName && formStyles.inputError]}
            value={displayName}
            onChangeText={setDisplayName}
            placeholder="e.g. Jana Al-Sayed"
            placeholderTextColor="#bbb"
            returnKeyType="next"
          />
          {errors.displayName && <Text style={formStyles.errorText}>{errors.displayName}</Text>}
        </View>

        {/* Tagline */}
        <View style={formStyles.field}>
          <Text style={formStyles.label}>Channel tagline *</Text>
          <TextInput
            style={[formStyles.inputEditable, errors.tagline && formStyles.inputError]}
            value={tagline}
            onChangeText={setTagline}
            placeholder="e.g. Making aerodynamics click for everyone"
            placeholderTextColor="#bbb"
            returnKeyType="next"
          />
          {errors.tagline && <Text style={formStyles.errorText}>{errors.tagline}</Text>}
        </View>

        {/* Subject focus */}
        <View style={formStyles.field}>
          <Text style={formStyles.label}>Subject focus * (pick all that apply)</Text>
          {errors.subjects && <Text style={formStyles.errorText}>{errors.subjects}</Text>}
          <View style={formStyles.chipGrid}>
            {SUBJECTS.map(s => (
              <TouchableOpacity
                key={s}
                style={[formStyles.chip, subjects.includes(s) && formStyles.chipSel]}
                onPress={() => toggleSubject(s)}
              >
                <Text style={[formStyles.chipText, subjects.includes(s) && formStyles.chipTextSel]}>{s}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
        ...
        <View style={formStyles.field}>
          ...
          <TouchableOpacity
            style={[formStyles.inputEditable, { flexDirection: "row", justifyContent: "space-between", alignItems: "center" }]}
            onPress={() => setExpPickerOpen(v => !v)}
          >
            <Text style={{ fontSize: 13, color: "#222" }}>{experience}</Text>
            <Ionicons name={expPickerOpen ? "chevron-up" : "chevron-down"} size={16} color="#aaa" />
          </TouchableOpacity>
          {expPickerOpen && (
            <View style={formStyles.picker}>
              {EXPERIENCE_OPTIONS.map(opt => (
                <TouchableOpacity
                  key={opt}
                  style={[formStyles.pickerOption, experience === opt && formStyles.pickerOptionSel]}
                  onPress={() => { setExperience(opt); setExpPickerOpen(false); }}
                >
                  <Text style={[formStyles.pickerOptionText, experience === opt && { color: "#2F54EB", fontWeight: "700" }]}>{opt}</Text>
                  {experience === opt && <Ionicons name="checkmark" size={16} color="#2F54EB" />}
                </TouchableOpacity>
              ))}
            </View>
          )}
        ...
        <View style={formStyles.field}>
          ...
          <TextInput
            style={[formStyles.inputEditable, formStyles.multiline, errors.whyTeach && formStyles.inputError]}
            value={whyTeach}
            onChangeText={setWhyTeach}
            placeholder="Tell us what drives you to share knowledge..."
            placeholderTextColor="#bbb"
            multiline
            numberOfLines={4}
            textAlignVertical="top"
          />
          <Text style={formStyles.charCount}>{whyTeach.length} chars (min 20)</Text>
          {errors.whyTeach && <Text style={formStyles.errorText}>{errors.whyTeach}</Text>}
        </View>

        <TouchableOpacity
          style={[formStyles.submitBtn, submitting && { opacity: 0.7 }]}
...
export default function ProfileScreen({ signOut, navigation }) {
  ...
  const [pendingCreatorData, setPendingCreatorData] = useState(null);
    ...
    useCallback(() => {
      ...
      (async () => {
        try {
          ...
          // If server marks this user as already a creator, sync context
          if (profile.isCreator && !isCreator) {
            await setCreatorProfile({
              creatorId: profile.creatorId ?? profile._id,
              displayName: profile.name,
              channelName: profile.channelName ?? profile.name,
  ...
  const handleLogout = () => {
    Alert.alert("Log out", "Are you sure you want to log out?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          await clearCreator(); // wipe creator state from AsyncStorage
          signOut();
        },
      },
    ]);
  ...
  // Called when API succeeds in CreatorFormScreen
  const handleFormSubmit = (creatorData) => {
    setPendingCreatorData(creatorData);
    setScreen("success");
  };

  // Called when user taps "Go to my creator profile" on success screen
  const activateCreator = async () => {
    await setCreatorProfile(pendingCreatorData);
  ...
  const renderTabContent = () => {
      ...
      return (
        <View style={mainStyles.emptyState}>
          <View style={mainStyles.emptyIconBox}>
            <Ionicons name={activeTab === 0 ? "videocam-outline" : "bookmark-outline"} size={26} color="#2F54EB" />
          </View>
      ...
      return (
        <View style={mainStyles.emptyState}>
          ...
          >
            ...
            <Text style={mainStyles.uploadBtnText}>{uploading ? "Uploading…" : "Upload video"}</Text>
          </TouchableOpacity>
        </View>
      );
    }
    ...
    return (
      <View>
        ...
        >
          {uploading ? <ActivityIndicator size="small" color="#fff" /> : <Ionicons name="add" size={16} color="#fff" />}
          <Text style={mainStyles.uploadBtnText}>{uploading ? "Uploading…" : "Upload new course"}</Text>
        </TouchableOpacity>
      </View>
    );
  };
...
const formStyles = StyleSheet.create({
  ...
  inputEditable: {
    ...
    fontSize: 13, color: "#222", minHeight: 42,
  },
  inputError: { borderColor: "#FF3B30" },
  multiline: { height: 100, textAlignVertical: "top" },
  errorText: { fontSize: 11, color: "#FF3B30", marginTop: 4 },
  charCount: { fontSize: 10, color: "#aaa", marginTop: 4, textAlign: "right" },
  ...
  pickerOptionSel: { backgroundColor: "#EEF2FF" },
