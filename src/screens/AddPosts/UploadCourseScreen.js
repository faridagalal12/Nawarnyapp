...
export default function UploadCourseScreen() {
  ...
  const pickVideo = async () => {
    try {
      ...
      setVideos(prev => [...prev, { title: fileName.replace(/\.[^/.]+$/, ""), videoUrl, order: prev.length + 1 }]);
      setProgress("");
      setUploading(false);
    } catch (err) {
      setUploading(false);
  ...
  const pickFile = async () => {
    try {
      ...
      setFiles(prev => [...prev, { name: file.name, fileUrl, type: file.mimeType ?? "document" }]);
      setProgress("");
      setUploading(false);
    } catch (err) {
      setUploading(false);
  ...
  const handleSubmit = async () => {
    ...
    if (videos.length === 0 && files.length === 0) { Alert.alert("No content", "Please add at least one video or file."); return; }
    setUploading(true);
    setProgress("Creating course...");
    try {
      await api.post("/creator/course", { title: title.trim(), description: description.trim(), category, price: parseFloat(price) || 0, videos, files });
      Alert.alert("Success! 🎉", "Your course has been published.", [{ text: "OK", onPress: () => navigation.goBack() }]);
    } catch (err) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <TextInput style={[styles.input, styles.textArea]} placeholder="What will students learn?" value={description} onChangeText={setDescription} multiline maxLength={500} />
        ...
        <TextInput style={styles.input} placeholder="0" value={price} onChangeText={setPrice} keyboardType="numeric" maxLength={6} />
        ...
        {videos.length === 0 ? (
          <View style={styles.emptySection}><Ionicons name="videocam-outline" size={28} color="#ccc" /><Text style={styles.emptySectionText}>No videos added yet</Text></View>
        ) : videos.map((v, i) => (
          <View key={i} style={styles.itemCard}>
            <View style={styles.itemIconBox}><Ionicons name="videocam-outline" size={20} color="#2F54EB" /></View>
            <Text style={styles.itemName} numberOfLines={1}>{v.title}</Text>
            <TouchableOpacity onPress={() => removeVideo(i)} style={styles.removeBtn}><Ionicons name="trash-outline" size={18} color="#ff4d4f" /></TouchableOpacity>
          </View>
        ))}
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>Files ({files.length})</Text>
          <TouchableOpacity style={styles.addBtn} onPress={pickFile} disabled={uploading}>
            <Ionicons name="add" size={18} color="#2F54EB" />
            <Text style={styles.addBtnText}>Add File</Text>
          </TouchableOpacity>
        </View>
        {files.length === 0 ? (
          <View style={styles.emptySection}><Ionicons name="document-outline" size={28} color="#ccc" /><Text style={styles.emptySectionText}>No files added yet</Text></View>
        ) : files.map((f, i) => (
          <View key={i} style={styles.itemCard}>
            <View style={styles.itemIconBox}><Ionicons name="document-outline" size={20} color="#2F54EB" /></View>
            <Text style={styles.itemName} numberOfLines={1}>{f.name}</Text>
            <TouchableOpacity onPress={() => removeFile(i)} style={styles.removeBtn}><Ionicons name="trash-outline" size={18} color="#ff4d4f" /></TouchableOpacity>
          </View>
        ))}
        {uploading && <View style={styles.progressRow}><ActivityIndicator color="#2F54EB" /><Text style={styles.progressText}>{progress}</Text></View>}
...
const styles = StyleSheet.create({
  ...
  addBtn:           { flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "#EEF2FF", borderRadius: 20, paddingHorizontal: 12, paddingVertical: 6 },
  addBtnText:       { fontSize: 13, color: "#2F54EB", fontWeight: "600" },
  emptySection:     { alignItems: "center", paddingVertical: 20, gap: 6, backgroundColor: "#fff", borderRadius: 12, marginBottom: 16, borderWidth: 1.5, borderColor: "#E2E8F0", borderStyle: "dashed" },
  emptySectionText: { fontSize: 13, color: "#aaa" },
  itemCard:         { flexDirection: "row", alignItems: "center", backgroundColor: "#fff", borderRadius: 12, padding: 12, marginBottom: 8, borderWidth: 1, borderColor: "#E2E8F0" },
  itemIconBox:      { width: 38, height: 38, borderRadius: 10, backgroundColor: "#EEF2FF", justifyContent: "center", alignItems: "center", marginRight: 10 },
  itemName:         { flex: 1, fontSize: 13, fontWeight: "600", color: "#1E293B" },
  removeBtn:        { padding: 6 },
  progressRow:      { flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: "#EEF2FF", borderRadius: 12, padding: 14, marginVertical: 12 },
  progressText:     { fontSize: 13, color: "#2F54EB", fontWeight: "500", flex: 1 },
  submitBtn:        { backgroundColor: "#2F54EB", borderRadius: 14, padding: 16, alignItems: "center", marginTop: 20 },
