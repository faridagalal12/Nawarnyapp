...
import {
  ActivityIndicator, Animated, Easing, SafeAreaView, ScrollView,
  StatusBar, StyleSheet, Text, TouchableOpacity, Vibration, View,
} from 'react-native';
...
const fallbackCategory = { label: 'Mission', icon: 'graduation-cap', family: 'FontAwesome5', color: BLUE };

function triggerHaptic(type) {
  if (type === 'success') { Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success).catch(() => Vibration.vibrate(70)); return; }
  if (type === 'error')   { Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error).catch(() => Vibration.vibrate([0,80,40,120])); return; }
  if (type === 'complete') {
    Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success).catch(() => Vibration.vibrate([0,90,50,150]));
    setTimeout(() => Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy).catch(() => Vibration.vibrate(120)), 180);
    return;
  }
...
function OptionCard({ option, selected, checked, correct, wrong, onPress }) {
  return (
    <TouchableOpacity
      ...
      activeOpacity={0.84} disabled={checked} onPress={onPress}
    >
...
function getAccuracy(correct, total) { return !total ? 0 : Math.round((correct / total) * 100); }
...
export default function GMissionScreen({ navigation, route }) {
  ...
  const [earnedBadges,    setEarnedBadges]    = useState(route?.params?.earnedBadges ?? 0);
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      ...
      try {
        ...
        setQuestions(mapped.length > 0 ? mapped : fallbackQuestions(category.label, level.id));
      } catch {
        setQuestions(fallbackQuestions(category.label, level.id));
      } finally {
  ...
  function fallbackQuestions(label, levelId) {
    return [
      { id: `${levelId}-1`, type: 'multipleChoice', prompt: `What is a key concept in ${label}?`,      correctAnswer: 'Practice and feedback', options: ['Practice and feedback','Ignoring mistakes','Random guessing','Skipping steps'], points: 10 },
      { id: `${levelId}-2`, type: 'multipleChoice', prompt: `Which approach works best for ${label}?`, correctAnswer: 'Structured learning',    options: ['Structured learning','Avoid challenges','Rush through content','Skip review'],   points: 10 },
      { id: `${levelId}-3`, type: 'multipleChoice', prompt: `How do you improve in ${label}?`,         correctAnswer: 'Consistent practice',    options: ['Consistent practice','Give up early','Avoid feedback','Memorize without understanding'], points: 10 },
    ];
  }
  ...
  useEffect(() => { setSecondsLeft(QUESTION_SECONDS); }, [question?.id]);

  useEffect(() => {
    if (checked || completed || !question) return;
    const timer = setInterval(() => {
      setSecondsLeft(v => {
        if (v <= 1) { clearInterval(timer); resolveQuestion(null, true); return 0; }
        return v - 1;
      });
    }, 1000);
  ...
  const completeLevel = async ({ nextCorrectCount, nextEarnedXp, nextBestStreak, nextResults }) => {
    ...
    // Save to backend — pass refresh timestamp so GWelcomeScreen re-fetches
    try {
      ...
      // Signal GWelcomeScreen to refresh
        ...
        <LinearGradient colors={['#1E9BFF', DEEP_BLUE, '#056FC9']} style={styles.screen}>
          <ScrollView style={styles.completeScroll} contentContainerStyle={styles.completeContent} showsVerticalScrollIndicator={false}>
            <View style={styles.topBar}>
              ...
              <CurrencyPill points={totalPoints} badges={earnedBadges} />
            </View>
...
const styles = StyleSheet.create({
  ...
  primaryButton: { minHeight: 62, borderRadius: 18, backgroundColor: YELLOW, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, paddingHorizontal: 18 },
